...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = (velocity_mag * velocity_mag) / (2 * accel_mag);  // <- Fixed This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.velocity.x = 0.0; // <- Fixed This
      goal.velocity.y = 0.0; // <- Fixed This
      goal.velocity.z = 0.0; // <- Fixed This
    ...
    if (distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE) { // <- Fixed this
