...
std::vector<State> MotionPlanner::generate_offset_goals(
  ...
  auto yaw = goal_state.rotation.yaw + M_PI_2;  // <- Fixed This
  ...
  for (int i = 0; i < _num_paths; ++i) {
    ...
    goal_offset.location.x += offset * std::cos(yaw);  // <- Fixed This
    goal_offset.location.y += offset * std::sin(yaw);  // <- Fixed This
